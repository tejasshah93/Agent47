...
GLvoid glmDraw1(GLMmodel* model, GLuint mode,char *drawonly,int todo,float x_axis,float y_axis,float z_axis,float j_flag,float ang_ll,float ang_rl,float ang_la,float ang_ra,float angle_axis)
{
    static GLuint i;
    static GLMgroup* group;
    static GLMtriangle* triangle;
    ...
    while (group) 
    {
        ...
        if(todo == 1)
        {
            ...
            glRotatef(angle_axis,0,1,0);
        }
        if(strcmp(group->name, " leftleg") == 0)
        {
            glTranslatef(0,0.08,0);
            glRotatef(ang_ll,1,0,0);
            if(j_flag == 1)
                ang_ll = 0;
        }
        ...
        {
            glTranslatef(0,0.08,0);
            glRotatef(ang_rl,1,0,0);
            if(j_flag == 1)
                ang_rl = 0;
        }

        if(strcmp(group->name, " leftarm") == 0)
        {
            glTranslatef(-0.2f,0.58f,-0.1f);
            glRotatef(ang_la,1,0,0);
            glTranslatef(0.2f,-0.58f,0.1f);
            if(j_flag == 1)         
                ang_la = 0;
            
        }
        if(strcmp(group->name, " rightarm") == 0)
        {
            glTranslatef(-0.2f,0.58f,-0.1f);
            glRotatef(ang_ra,1,0,0);
            glTranslatef(0.2f,-0.58f,0.1f);
            if(j_flag == 1)         
                ang_ra = 0;
